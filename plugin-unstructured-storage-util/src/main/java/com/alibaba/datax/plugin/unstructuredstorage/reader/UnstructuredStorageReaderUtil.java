...
public class UnstructuredStorageReaderUtil {
     ...
     */
    public static String[] splitOneLine(String inputLine,
                                        char delimiter) {
    ...
    /**
      ...
      * @throws IOException
     */
    public static String[] splitBufferedReader(CsvReader csvReader) throws IOException {
        String[] splitedResult = null;
        if (csvReader.readRecord()) {
            splitedResult = csvReader.getValues();
        }
     ...
     * 从流中读取数据信息
     * 
     * @param inputStream 非数据结构数据流，ftp,sftp,hdfs等
     * @param context 文件名?
     * @param readerSliceConfig reader 配置
     * @param recordSender 需要送交 channel 的 source
     * @param taskPluginCollector 异常数据收集器
     */
    public static void readFromStream(InputStream inputStream,
        ...
        /* 压缩文件处理，此时未考虑压缩文件中不同  encoding 的情况 */
        ...
        try {
                ...
                if ("lzo_deflate".equalsIgnoreCase(compress)) {
                  ...
                  reader = new BufferedReader(new InputStreamReader(
                    ...
                    throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.ILLEGAL_VALUE,
                                                          String.format(
                                                                        "仅支持 gzip, bzip2, zip, lzo, lzo_deflate, hadoop-snappy, framing-snappy"
                                                                            + "文件压缩格式 , 不支持您配置的文件压缩格式: [%s]",
                                                                        compress));
                }
            }
            ...
            throw DataXException
                .asDataXException(UnstructuredStorageReaderErrorCode.OPEN_FILE_WITH_CHARSET_ERROR, String.format("不支持的编码格式 : [%s]", encoding), uee);
    ...
    /**
     * 处理从流中读取的数据信息
      * @param reader 非数据结构 bufferReader 流
      * @param context 文件名?
      * @param readerSliceConfig reader 配置
      * @param recordSender 需要送交 channel 的 source
      * @param taskPluginCollector 异常数据收集器
     */
    public static void doReadFromStream(BufferedReader reader,
                                        String context,
                                        Configuration readerSliceConfig,
                                        RecordSender recordSender,
                                        TaskPluginCollector taskPluginCollector) {
        ...
        // 定义数据 为 null 时的字段名，eg:例如如果用户配置: "nullFormat":"\N"，那么如果数据源是"\N"，将视作 null 字段
        ...
        try {
            ...
            throw DataXException
                .asDataXException(UnstructuredStorageReaderErrorCode.READ_FILE_IO_ERROR, String.format("读取文件错误 : [%s]", context), ioe);
        } catch (Exception e) {
            throw DataXException
                .asDataXException(UnstructuredStorageReaderErrorCode.RUNTIME_EXCEPTION, String.format("运行时异常 : %s", e.getMessage()), e);
        } finally {
            csvReader.close();
            IOUtils.closeQuietly(reader);
        }
     ...
     * 将数据转换成相应 records
      * @param recordSender 需要送交 channel 的 source
      * @param columnConfigs 复杂列实体类 list
      * @param sourceLine 切割后一行数据的字符串数组
      * @param nullFormat 数据 为 null 时的字段名
      * @param taskPluginCollector 异常数据收集器
      * @return Record
     */
    public static Record transportOneRecord(RecordSender recordSender,
                                            List<ColumnEntry> columnConfigs,
                                            String[] sourceLine,
                                            String nullFormat,
                                            TaskPluginCollector taskPluginCollector) {
        ...
        if (null == columnConfigs || columnConfigs.size() == 0) {
            // 创建都为 String 类型 column 的 record
            ...
            try {
                ...
                for (ColumnEntry columnConfig : columnConfigs) {
                    String columnType = columnConfig.getType();     // 类型
                    Integer columnIndex = columnConfig.getIndex();  // 读取值的列号
                    String columnConst = columnConfig.getValue();   // 视为常量，自动填充到列
    ...
    /**
     * 将 cloumn node 转换成列实体，主要处理复杂列配置，eg:"column": [{"index": 0,"type": "long"}]
      * @param configuration 配置项
      * @param path 文件路径
      * @return ColumnEntry 实体类 list
     */
    ...
    public static void validateEncoding(Configuration readerConfiguration) {
        // encoding check
        String encoding = readerConfiguration.getString(com.alibaba.datax.plugin.unstructuredstorage.reader.Key.ENCODING,
        ...
        try {
            ...
            throw DataXException
                .asDataXException(UnstructuredStorageReaderErrorCode.ILLEGAL_VALUE, String.format("不支持您配置的编码格式 : [%s]", encoding), uce);
        } catch (Exception e) {
            throw DataXException
                .asDataXException(UnstructuredStorageReaderErrorCode.CONFIG_INVALID_EXCEPTION, String.format("编码配置异常, 请联系我们: %s", e.getMessage()), e);
        }
    }

    public static void validateCompress(Configuration readerConfiguration) {
        String compress = readerConfiguration.getUnnecessaryValue(com.alibaba.datax.plugin.unstructuredstorage.reader.Key.COMPRESS, null, null);
        if (StringUtils.isNotBlank(compress)) {
            compress = compress.toLowerCase().trim();
            boolean compressTag = "gzip".equals(compress) || "bzip2".equals(compress) || "zip".equals(compress) || "lzo".equals(compress)
                || "lzo_deflate".equals(compress) || "hadoop-snappy".equals(compress) || "framing-snappy".equals(compress);
            if (!compressTag) {
                throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.ILLEGAL_VALUE,
                                                      String.format(
                                                                    "仅支持 gzip, bzip2, zip, lzo, lzo_deflate, hadoop-snappy, framing-snappy "
    ...
    public static void validateFieldDelimiter(Configuration readerConfiguration) {
        ...
        if (null == delimiterInStr) {
            throw DataXException
                .asDataXException(UnstructuredStorageReaderErrorCode.REQUIRED_VALUE,
                                  String.format("您提供配置文件有误，[%s]是必填参数.", com.alibaba.datax.plugin.unstructuredstorage.reader.Key.FIELD_DELIMITER));
    ...
    /**
           *  配置 csv 属性
      * @param readerConfiguration reader 的配置 json
     */
    ...
    /**
     *
     * @Title: getRegexPathParent @Description: 获取正则表达式目录的父目录 @param @param regexPath @param @return @return
     *         String @throws
     */
    ...
    /**
     *
     * @Title: getRegexPathParentPath @Description: 获取含有通配符路径的父目录，目前只支持在最后一级目录使用通配符*或者?. (API
     *         jcraft.jsch.ChannelSftp.ls(String path)函数限制)
     *         http://epaul.github.io/jsch-documentation/javadoc/ @param @param regexPath @param @return @return
     *         String @throws
     */
    ...
    public static void setCsvReaderConfig(CsvReader csvReader) {
        if (null != UnstructuredStorageReaderUtil.csvReaderConfigMap && !UnstructuredStorageReaderUtil.csvReaderConfigMap.isEmpty()) {
            try {
                // 遍历 map 的 key，如果 bean 中有这个属性，则把对应的 value 值赋给该属性
                ...
                LOG.info(String
                    .format("WARN!!!!忽略csvReaderConfig配置!通过BeanUtils.populate配置您的csvReaderConfig发生异常,您配置的值为: %s;请检查您的配置!CsvReader使用默认值[%s]",
                            JSON.toJSONString(UnstructuredStorageReaderUtil.csvReaderConfigMap),
                            JSON.toJSONString(csvReader)));
            }
        } else {
            // 默认关闭安全模式, 放开10W字节的限制
            csvReader.setSafetySwitch(false);
            LOG.info(String.format("CsvReader使用默认值[%s],csvReaderConfig值为[%s]",
                                   JSON.toJSONString(csvReader),
                                   JSON.toJSONString(UnstructuredStorageReaderUtil.csvReaderConfigMap)));
        }
    }
