    ...
    public static class Job extends Reader.Job {
        ...
        private void validateParameter() {
            ...
            if (!connectPatternTag) {
                throw DataXException.asDataXException(FtpReaderErrorCode.ILLEGAL_VALUE, String.format("不支持您配置的ftp传输模式: [%s]", connectPattern));
            } else {
        ...
        public void destroy() {
            try {
                ...
                String message = String.format("关闭与ftp服务器连接失败: [%s] host=%s, username=%s, port=%s", e.getMessage(), host, username, port);
                LOG.error(message, e);
            }
        }
        ...
        public void startRead(RecordSender recordSender) {
            ...
            for (String fileName : this.sourceFiles) {
                ...
                UnstructuredStorageReaderUtil
                    .readFromStream(inputStream, fileName, this.readerSliceConfig, recordSender, this.getTaskPluginCollector());
                recordSender.flush();
            }
